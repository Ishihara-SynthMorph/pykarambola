%% REVISED MANUSCRIPT — clean version (no markup)
%% Addresses issues #162, #163, #164, #165, #166, #167, #169
%% Issues #168 and #170 include placeholder text pending figure generation.
...
\caption{%
  ...
  Three \textbf{alternative} input types converge at the shared core (\texttt{minkowski\_tensors});
  only one entry path is required for a given analysis:
  (1)~mesh files (\texttt{.poly}, \texttt{.off}, \texttt{.obj}, \texttt{.glb}, \texttt{.stl})
  via format-specific parsers (\texttt{parse\_*\_file});
  (2)~\texttt{Triangulation} objects (vertex and face arrays as NumPy arrays) passed directly;
  or (3)~3D integer label images via \texttt{minkowski\_tensors\_from\_label\_image},
  ...
  see~\cite{SchroderTurk2013} for exact expressions and Supplementary Table~\ref{tab:tensors} for integral definitions):
  ...
  \textit{Alt text: schematic diagram with three left-side input boxes (mesh file, NumPy arrays, label image) connected by arrows to a central processing box (minkowski\_tensors), which connects to three right-side output boxes (scalars, vectors, rank-2 tensors).}
}
\label{fig:pipeline}
\end{figure*}
...
% Issue #168 (annulus/genus>0 validation) and #170 (orientation-bias experiment):
% text and figures to be added here once notebooks and results are available.
...
Case~(ii) answers a fundamentally different morphometric question than case~(i): object~1 ($V = 56~\mu\text{m}^3$, $\chi = -12$, $\beta_0^{2,0} = 0.194$) is approximately twice the volume of objects~2 and~3 ($V \approx 27~\mu\text{m}^3$, $\chi = 2$ each), which are more elongated ($\beta_0^{2,0} \approx 0.144$--$0.151$).
The Euler characteristics ($\chi = -12$ for object~1 and $\chi = 2$ for objects~2--3) are consistent with the additivity relation: the aggregate in case~(i) gives $\chi = -12 + 2 + 2 = -8$, matching the single-label result.
...
To demonstrate the applicability of the label-image API in a complete segmentation-to-morphometry pipeline, Supplementary Figure~\ref{fig:workflow} shows an end-to-end analysis of 3D confocal nuclei: a raw fluorescence stack is segmented to produce per-nucleus integer labels, Minkowski tensors are computed for all nuclei in a single call, and the resulting feature vectors (scalars, anisotropy indices, eigenvalues, and Minkowski structure metrics) are projected by PCA and used to cluster nuclei by shape, with meshes coloured by cluster assignment.
The full pipeline is provided in \texttt{segmentation\_workflow.ipynb} in the project repository.
...
Users encountering open or non-manifold meshes in their own datasets can repair them using established mesh-processing tools such as MeshLab~\cite{Cignoni2008meshlab}, Open3D~\cite{Zhou2018open3d}, or PyVista~\cite{Sullivan2019pyvista} prior to passing the mesh to pykarambola.
...
\textit{Alt text: two line plots on log-log axes. Left: wall-clock time versus mesh size for three implementations; all three lines rise together, with the Cython build closest to or below the C++ line. Right: speedup ratio versus mesh size; the Cython line sits near or above 1, the pure-Python line near 0.9.}}
\label{fig:runtime}
\end{figure*}
...
\caption{Numerical accuracy of \texttt{pykarambola} relative to C++ \texttt{karambola} at $28^3$ voxel resolution.
...
the AdrenalMNIST3D dataset (1{,}584 meshes).
...
\textit{Alt text: grid of scatter plots, one per Minkowski feature, with C++ karambola values on the x-axis and pykarambola values on the y-axis. All panels show points tightly along the red identity diagonal, confirming near-identical outputs across all 121 features and 1{,}584 meshes.}}
\label{fig:accuracy}
\end{figure*}
...
\caption{%
  ...
  one tensor set describes the aggregate of all three DNA objects ($V=110$\,\textmu m$^3$, $A=435$\,\textmu m$^2$, $\chi=-8$, $\beta=0.118$).
  ...
  \textit{Alt text: two 3D mesh renders of chromosomal DNA. Left panel (a): a single blue semi-transparent mesh of the aggregate chromatin mass, annotated with V, A, chi, and beta values. Right panel (b): three separately coloured meshes (green, orange, purple) for individual DNA objects, each annotated with its own V, A, chi, and beta values and a centroid cross marker.}
}
\label{fig:labelimage}
\end{figure*}

%% -------------------------------------------------------
%% Supplementary Figure S5 — End-to-end confocal nuclei workflow (#164)
%% -------------------------------------------------------
...
\caption{%
  ...
  (\textbf{c}) PCA of the full Minkowski tensor feature vector (scalars, anisotropy indices $\beta$, eigenvalues, and Minkowski structure metrics) computed for all nuclei in a single \texttt{minkowski\_tensors\_from\_label\_image} call.
  (\textbf{d}) 3D meshes of segmented nuclei coloured by shape cluster, revealing morphologically distinct subpopulations.
  The complete pipeline is provided in \texttt{examples/segmentation\_workflow.ipynb}.
  \textit{Alt text: four-panel figure. Top-left: grey-scale 3D confocal image of cell nuclei. Top-right: the same nuclei as a colour-coded integer segmentation. Bottom-left: 2D PCA scatter plot of Minkowski tensor features, points coloured by cluster. Bottom-right: 3D mesh render of nuclei coloured by cluster assignment.}
}
\label{fig:workflow}
\end{figure*}

%% -------------------------------------------------------
%% Supplementary Table S1 — Minkowski tensor definitions (#165)
%% -------------------------------------------------------
...
\caption{%
  ...
  Notation: $K$ denotes the body, $\partial K$ its triangulated surface, $f$ a triangular face with area $A_f$ and unit outward normal $\hat{n}_f$, $e$ a mesh edge with length $\ell_e$ and exterior dihedral angle $\alpha_e$, and $v$ a vertex with angle deficit $\delta_v$.
  All tensors carry the Steiner normalisation factor $1/3$.
  Derived scalars (anisotropy index $\beta$, trace, trace ratio) and eigensystems are returned with \texttt{compute=`all'}.
  See \texttt{docs/minkowski\_tensors.md} in the project repository for discrete mesh formulae and full derivations.
}
\label{tab:tensors}
\small
\begin{tabular}{llll}
...
\texttt{w000} & $W_0^{0,0}$ & $\int_K dV$ & Volume $V$ \\
\texttt{w100} & $W_1^{0,0}$ & $\frac{1}{3}\int_{\partial K} dA$ & Surface area $A/3$ \\
...
\texttt{w010} & $W_0^{1,0}$ & $\int_K \mathbf{x}\,dV$ & Volume moment; $\mathtt{w010}/\mathtt{w000}$ = centroid \\
\texttt{w110} & $W_1^{1,0}$ & $\frac{1}{3}\int_{\partial K} \mathbf{x}\,dA$ & Surface moment; $/\mathtt{w100}$ = area centroid \\
\texttt{w210} & $W_2^{1,0}$ & $\frac{1}{3}\int_{\partial K} H\,\mathbf{x}\,dA$ & Mean-curvature-weighted centroid \\
\texttt{w310} & $W_3^{1,0}$ & $\frac{1}{3}\int_{\partial K} K_G\,\mathbf{x}\,dA$ & Gaussian-curvature-weighted centroid \\
\hline
\multicolumn{4}{l}{\textit{Rank-2 position tensors} $W_\nu^{2,0}$ (symmetric $3\times3$ matrices)} \\
\texttt{w020} & $W_0^{2,0}$ & $\int_K x_i x_j\,dV$ & Solid moment tensor; $\beta$ = anisotropy \\
\texttt{w120} & $W_1^{2,0}$ & $\frac{1}{3}\int_{\partial K} x_i x_j\,dA$ & Hollow (surface) moment tensor \\
\texttt{w220} & $W_2^{2,0}$ & $\frac{1}{3}\int_{\partial K} H\,x_i x_j\,dA$ & Wire (curvature-weighted) moment tensor \\
\texttt{w320} & $W_3^{2,0}$ & $\frac{1}{3}\int_{\partial K} K_G\,x_i x_j\,dA$ & Vertex moment tensor \\
\hline
\multicolumn{4}{l}{\textit{Rank-2 normal tensors} $W_\nu^{0,2}$ (symmetric $3\times3$ matrices)} \\
\texttt{w102} & $W_1^{0,2}$ & $\frac{1}{3}\int_{\partial K} n_i n_j\,dA$ & Normal distribution tensor; $\beta=1$ for sphere \\
\texttt{w202} & $W_2^{0,2}$ & $\frac{1}{3}\int_{\partial K} H\,n_i n_j\,dA$ & Curvature-weighted normal tensor \\
\hline
\multicolumn{4}{l}{\textit{Higher-rank tensors} (\texttt{compute=`all'} only)} \\
\texttt{w103} & $W_1^{0,3}$ & $\frac{1}{3}\int_{\partial K} n_i n_j n_k\,dA$ & Rank-3 normal tensor; zero for centrosymmetric shapes \\
\texttt{w104} & $W_1^{0,4}$ & $\frac{1}{3}\int_{\partial K} n_i n_j n_k n_l\,dA$ & Rank-4 normal tensor; related to MSM \\
\texttt{msm\_ql} & $q_\ell$ & $\sqrt{\frac{4\pi}{2\ell+1}\sum_m|Q_{\ell m}|^2}$ & $\ell$-fold orientational order strength \\
\texttt{msm\_wl} & $w_\ell$ & $\propto \tilde{w}_\ell^{1/3}$ (Wigner $3j$ sum) & Phase structure of orientational order \\
\hline
\end{tabular}
\end{table*}
