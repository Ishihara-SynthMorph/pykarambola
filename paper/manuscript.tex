...
\replaced{We extended this accuracy assessment to the full 121-feature output on a large biological dataset, comparing pykarambola against C++ karambola on the AdrenalMNIST3D }{To assess numerical accuracy, we compared all 121 output features of pykarambola against C++ karambola on the full AdrenalMNIST3D }
...
pykarambola's implementation uses the Racah formula~\cite{Varshalovich1988},
...
\caption{%
  \added{Numerical accuracy of pykarambola relative to C++ karambola on three synthetic meshes of known topology.
  Each panel shows a scatter plot of values from identical analyses run in C++ (karambola, $x$-axis) and Python (pykarambola, $y$-axis).
