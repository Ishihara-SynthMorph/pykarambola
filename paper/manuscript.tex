%% Single-source manuscript — compile two ways:
%%   make marked       → manuscript.pdf        (tracked changes visible)
%%   make clean_pdf    → manuscript_clean.pdf  (clean, no markup)
%% Issue #168: resolved — topology validation notebook and supplementary figure added.
%% Issue #170: resolved — orientation-bias experiment and supplementary figure added.
\documentclass[times, twoside, watermark]{zHenriquesLab-StyleBioRxiv}

% Pass \def\FINAL{} on the command line (via Makefile) for the clean version.
% Without it, the marked version is produced.
\ifdefined\FINAL
  % Clean version: skip the changes package entirely to avoid xkeyval
  % sanitizer failures inside captions.  Define minimal stubs that accept
  % the same optional-author argument as the real commands.
  \newcommand{\newtxt}[1]{#1}
  \newcommand{\added}[2][]{#2}        % show added text
  \newcommand{\deleted}[2][]{}        % hide deleted text
  \newcommand{\replaced}[3][]{#2}     % show new text, hide old text
\else
  \usepackage[deletedmarkup=sout,authormarkup=none,commandnameprefix=ifneeded]{changes}
  \definecolor{chgadded}{RGB}{34,139,34}
  \definecolor{chgdeleted}{RGB}{180,0,0}
  \colorlet{Changes@Color@deleted}{chgdeleted}
  \newcommand{\newtxt}[1]{\textcolor{chgadded}{#1}}
  \renewcommand{\added}[2][]{\newtxt{#2}}  % consistent green, no underline
\fi
...
\author[1, \Letter]{Yajushi Khurana}
\author[1,2,\Letter]{Keisuke Ishihara}

\affil[1]{Department of Computational and Systems Biology, School of Medicine, University of Pittsburgh, Pittsburgh, PA, USA}
\affil[2]{Institute for Synthetic Biology, University of Pittsburgh, Pittsburgh, PA, USA}
\maketitle
...
A high-level label-image API converts 3D integer arrays into per-object Minkowski tensors in a single call, making pykarambola directly compatible with the output of widely used segmentation tools.
An optional Cython extension accelerates graph-traversal steps of mesh initialization for large-scale analyses.
Validated on synthetic meshes spanning three topological classes (Euler characteristics $\chi = 0$, $-2$, $+4$) and benchmarked on 1{,}584 adrenal gland meshes, pykarambola reproduces all 121 C++ karambola output features to near-floating-point agreement and, in the pure-Python build, is $2.8\times$ faster at $28^3$ and $1.5\times$ faster at $64^3$ voxel resolution, with speedups primarily attributable to karambola's sequential per-object file I/O.
...
Standard descriptors such as volume, surface area, sphericity, and inertia tensor eigenvalues (as computed in scikit-image~\cite{vanderwalt2014scikit}) capture the geometry of the mass distribution but not the directed properties of the surface itself: its local curvature, normal orientation, and their spatial distribution.
Spherical harmonics-based descriptors encode richer structural information~\cite{Kazhdan2003, viana2023integrated}, but are restricted to genus-zero surfaces and cannot describe objects with handles, tunnels, or enclosed voids, which arise in multi-component segmentations and topologically complex organelles.
...
The classical Minkowski functionals form a complete basis for continuous, additive, and motion-invariant scalar shape measures: by Hadwiger's theorem, any such functional on convex bodies in $\mathbb{R}^3$ is a linear combination of exactly four measures~\cite{Hadwiger1957}: volume $V$, surface area $A$, integrated mean curvature $H$, and Euler characteristic $\chi$, a topological invariant equal to the number of connected components minus the number of handles plus the number of enclosed voids.
...
\newtxt{Minkowski tensor analysis has been applied across biological length scales and structural contexts: from the local and global geometry of trabecular bone~\cite{Callens2021} and the morphology of individual neuronal cells~\cite{Beisbart2006} to the collective shape ordering of epithelial tissues~\cite{Happel2025}, demonstrating that the framework is broadly applicable regardless of the type of biological structure under study.}
A C++ implementation, karambola~\cite{Schaller2020karambola}, has been applied to characterize anisotropy in cellular foams, granular packings, and porous materials~\cite{SchroderTurk2013}, but is not accessible within the Python bioimage ecosystem.
...
Numerical correctness of the port was further validated by benchmarking against the C++ karambola binary on synthetic meshes spanning three topological classes (torus, holey slab, hollow shell) with known analytical Minkowski values, and on a large biological dataset, confirming near-floating-point agreement for all Minkowski scalars, vectors, and tensors.
...
\caption{%
  ...
  \replaced{Three \textbf{alternative} input types converge at the shared core (\texttt{minkowski\_tensors}); only one entry path is required for a given analysis: (1)~mesh files (\texttt{.poly}, \texttt{.off}, \texttt{.obj}, \texttt{.glb}, \texttt{.stl}) via format-specific parsers (\texttt{parse\_*\_file}); (2)~\texttt{Triangulation} objects (vertex and face arrays as NumPy arrays) passed directly; or (3)~3D integer label images via \texttt{minkowski\_tensors\_from\_label\_image}, which extracts a triangulated isosurface per label using \texttt{skimage.marching\_cubes}.}{Three entry paths converge at the shared core (\texttt{minkowski\_tensors}): mesh files (\texttt{.poly}, \texttt{.off}, \texttt{.obj}, \texttt{.glb}, \texttt{.stl}) via format-specific parsers (\texttt{parse\_*\_file}); \texttt{Triangulation} objects (vertex and face arrays) passed directly; and 3D integer label images via \texttt{minkowski\_tensors\_from\_label\_image}, which extracts a triangulated isosurface per label using \texttt{skimage.marching\_cubes}.}
...
up to 2.6M vertices; Supplementary Figure~\ref{fig:runtime}b), with both within 3\% of
...
across all mesh sizes (Supplementary Figure~\ref{fig:runtime}b).
...
As a direct test of correctness on topologically non-trivial surfaces, we evaluated \texttt{pykarambola} and C++ \texttt{karambola} on three synthetic meshes with known analytical Minkowski scalar values: a parametric torus ($\chi = 0$, genus~1; $R = 3$, $r = 1$), a rectangular slab with two rectangular through-holes ($\chi = -2$, genus~2), and two nested icospheres forming a hollow shell ($\chi = +4$; $R_\mathrm{out} = 2$, $R_\mathrm{in} = 1$).
Each shape was tested at five mesh resolutions.
Across all shapes, resolutions, and feature types (scalars, vectors, rank-2 tensors), both implementations fall on the identity line (Supplementary Figure~\ref{fig:topology}).
At the finest resolution, the torus scalar functionals converge to within 0.08\% of their analytical values: $W_0^{0,0} = 59.17$ vs.\ $2\pi^2 Rr^2 = 59.22$ (0.08\%); $W_1^{0,0} = 39.46$ vs.\ $(4/3)\pi^2 Rr = 39.48$ (0.03\%); $W_2^{0,0} = 19.737$ vs.\ $2\pi^2 R/3 = 19.739$ (0.01\%).
The hollow shell scalar functionals agree with their analytical expressions to within 0.003\%.
For all three shapes, $W_3^{0,0}$ reproduces the exact topological value $(2\pi/3)\chi$ to within $10^{-11}$; the holey slab additionally returns $W_0^{0,0} = 30.000$ and $W_1^{0,0} = 88/3 = 29.333$ to machine precision at all resolutions, because it is constructed from flat faces.
Minkowski vector functionals ($W_\nu^{1,0}$) are analytically zero for all three centrosymmetric shapes; both implementations return values below $10^{-8}$, and the residual differences between them reflect independent floating-point rounding rather than a systematic discrepancy.

We extended this accuracy assessment to the full 121-feature output on a large biological dataset, comparing \texttt{pykarambola} against C++ \texttt{karambola} on the AdrenalMNIST3D
...
each scatter plot (Supplementary Figure~\ref{fig:accuracy}).
...
They show a distinct accuracy pattern (Supplementary Figure~\ref{fig:accuracy}).
...
Supplementary Figure~\ref{fig:labelimage} illustrates how the choice of labeling scheme directly determines what the tensors measure, using the DNA channel as an example.
In case~(i), the binary DNA mask is cast to an integer array (\texttt{dna\_mask.astype(int)}) and passed with the default \texttt{autolabel=False}; because all non-zero voxels share label value~1, the function returns one tensor set describing the aggregate geometry of all chromosomal material ($V = 110~\mu\text{m}^3$, $A = 435~\mu\text{m}^2$, \replaced{$\chi = -8$ (genus~5, consistent with the convoluted topology of condensed chromosomal material in anaphase)}{$\chi = 2$ (a topologically closed, simply connected surface)}, $\beta_0^{2,0} = 0.118$).
...
Case~(ii) answers a fundamentally different morphometric question than case~(i): object~1 (\replaced{$V = 56~\mu\text{m}^3$, $\chi = -12$, $\beta_0^{2,0} = 0.194$}{$V = 56~\mu\text{m}^3$, $\beta_0^{2,0} = 0.194$}) is approximately twice the volume of objects~2 and~3 (\replaced{$V \approx 27~\mu\text{m}^3$, $\chi = 2$ each}{$V \approx 27~\mu\text{m}^3$ each}), which are more elongated ($\beta_0^{2,0} \approx 0.144$--$0.151$).
\added{The Euler characteristics ($\chi = -12$ for object~1 and $\chi = 2$ for objects~2--3) are consistent with the additivity relation: the aggregate in case~(i) gives $\chi = -12 + 2 + 2 = -8$, matching the single-label result.}
...
\added{To demonstrate the applicability of the label-image API in a complete segmentation-to-morphometry pipeline, Supplementary Figure~\ref{fig:workflow} shows an end-to-end analysis of 3D confocal nuclei: a raw fluorescence stack is segmented to produce per-nucleus integer labels, Minkowski tensors are computed for all nuclei in a single call, and the resulting feature vectors are projected by PCA and used to cluster nuclei by shape, with meshes coloured by cluster assignment.
The full pipeline is provided in \texttt{examples/segmentation\_workflow.ipynb} in the project repository.}
...
The hiPSC anaphase example in the Results section demonstrates that this analysis is accessible directly from the integer label arrays produced by standard segmentation tools, without any intermediate mesh preparation.
...
Numerical agreement between \texttt{pykarambola} and C++ \texttt{karambola} was confirmed on both topologically non-trivial synthetic surfaces with known analytical values ($\chi = 0$, $-2$, $+4$) and across the full 121-feature output on 1{,}584 biological meshes, with all outputs falling on the identity line and maximum mean absolute difference of $1.6\times10^{-4}$.
...
For smooth objects, tensor values converge asymptotically as voxel resolution increases, but rank-2 surface tensors are subject to an orientation bias from the anisotropy of the digital grid that can be significant at typical fluorescence microscopy resolutions \cite{Svane2017}; an alternative Voronoi-based approach that is asymptotically unbiased for binary voxel inputs without requiring surface reconstruction has recently been proposed \cite{Hug2025}.
\added{To quantify this bias, we rotated a synthetic triaxial ellipsoid (semi-axes 10, 6, 4~$\mu$m) through 12 orientations at four voxel sizes (0.25--2.0~$\mu$m) and measured the variation in $\beta$ and principal axis orientation across orientations (Supplementary Figure~\ref{fig:orientation_bias}).
At $\geq$40 voxels per major semi-axis (0.25~$\mu$m), $\beta$ varies by less than 0.4\% across orientations and the principal axis error remains below 0.2\textdegree.
At 10 voxels per major semi-axis (1.0~$\mu$m, typical of confocal imaging of $\sim$10~$\mu$m organelles), $\beta$ varies by $\pm$9\% and axis errors reach 1.4\textdegree; at 5 voxels per semi-axis these grow to $\pm$10\% and 2.7\textdegree.
We therefore recommend that objects of interest span at least 20 voxels across their longest dimension before tensor anisotropy values are interpreted quantitatively.}
...
\newtxt{Users encountering open or non-manifold meshes in their own datasets can repair them using established mesh-processing tools such as MeshLab~\cite{Cignoni2008meshlab}, Open3D~\cite{Zhou2018open3d}, or PyVista~\cite{Sullivan2019pyvista} prior to passing the mesh to pykarambola.}
...
Integration into interactive analysis environments is a particularly attractive direction, for example as a napari~\cite{Sofroniew2026napari} plugin that visualizes tensor outputs alongside raw image data and allows users to explore morphometric results in real time.
...
The AdrenalMNIST3D data is part of MedMNIST~\cite{Yang2023MedMNIST} and can be downloaded via the \texttt{medmnist} Python package.
...
pykarambola was developed by directly referencing the source code of karambola, the C++ reference implementation by Schaller, Kapfer, and Schröder-Turk~\cite{Schaller2020karambola}.
...
All panels and groupings are identical to Supplementary Figure~\ref{fig:accuracy}.}
\label{fig:accuracy_supp}
\end{figure*}

\begin{figure*}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{notebooks/results/scatter_topology_accuracy.pdf}
\caption{Numerical accuracy of \texttt{pykarambola} relative to C++ \texttt{karambola} on three synthetic meshes of known topology.
Each panel shows a scatter plot of values from identical analyses run in C++ (\texttt{karambola}, $x$-axis) and Python (\texttt{pykarambola}, $y$-axis).
Rows correspond to shapes: torus ($\chi = 0$, genus~1), holey slab ($\chi = -2$, genus~2), and hollow shell ($\chi = +4$, two components).
Columns correspond to feature groups: scalars ($W_\nu^{0,0}$), vectors ($W_\nu^{1,0}$), rank-2 position tensors ($W_\nu^{2,0}$), and rank-2 normal tensors ($W_\nu^{0,2}$).
The red diagonal indicates perfect agreement; all scalar and tensor points fall on this line.
Vector functionals ($W_\nu^{1,0}$) are analytically zero for these centrosymmetric shapes; residual spread in the vector panels reflects independent floating-point rounding in the two implementations rather than systematic error.
Each shape was tested at five mesh resolutions; each point represents one feature from one mesh.
\textit{Alt text: a 3-by-4 grid of scatter plots. Rows correspond to torus, holey slab, and hollow shell; columns correspond to scalar, vector, rank-2 position tensor, and rank-2 normal tensor feature groups. In each panel the $x$-axis shows karambola values and the $y$-axis shows pykarambola values; a red diagonal line marks perfect agreement. Scalar and tensor panels show points tightly clustered on the diagonal. Vector panels show points scattered near the origin, reflecting values close to zero for centrosymmetric shapes.}}
\label{fig:topology}
\end{figure*}
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{notebooks/results/orientation_bias.pdf}
\caption{Orientation bias introduced by marching-cubes surface extraction on a regular voxel grid.
A synthetic triaxial ellipsoid (semi-axes $A=10$, $B=6$, $C=4$~$\mu$m) was rotated through 12 orientations (0\textdegree--165\textdegree{} in 15\textdegree{} steps) and discretized at four voxel sizes.
\textbf{(a)} Anisotropy index $\beta = \lambda_\mathrm{min}/\lambda_\mathrm{max}$ of the normal-distribution tensor $W_1^{0,2}$ as a function of rotation angle; the dashed black line shows the reference value computed at 0.1~$\mu$m voxel size ($\approx$100 voxels per semi-axis).
\textbf{(b)} $\beta$ range (percentage of true $\beta$) and maximum principal-axis error across all orientations as a function of voxel size.
At $\geq$40 voxels per major semi-axis (0.25~$\mu$m), the $\beta$ range is $<$0.4\% and axis errors are $<$0.2\textdegree.
At 10 voxels per major semi-axis (1.0~$\mu$m), the $\beta$ range rises to 9\% and axis errors reach 1.4\textdegree.
\textit{Alt text: two-panel figure. Left panel: line plot of anisotropy index beta versus rotation angle (0 to 165 degrees) for four voxel sizes (0.25, 0.5, 1.0, 2.0 micrometres, coloured blue to orange), with a dashed black horizontal reference line; the four lines become increasingly spread and variable at coarser voxel sizes. Right panel: dual-axis line plot showing beta range (percentage of true beta, blue circles) and maximum principal-axis error in degrees (orange squares) versus voxel size; both quantities increase steeply from 0.25 to 2.0 micrometres.}}
...
\caption{%
  ...
  of a hiPSC in anaphase/telophase (Allen Cell Collection~\cite{viana2023integrated};
  isotropic voxel size 0.108\,\textmu m).
  ...
  one tensor set describes the aggregate of all three DNA objects \added{($V=110$\,\textmu m$^3$, $A=435$\,\textmu m$^2$, $\chi=-8$, $\beta=0.118$)}.
  ...
  three independent tensor sets, one per DNA object \added{(Object~I: $\chi=-12$; Objects~II--III: $\chi=2$ each)}.
...
% Issue #164: pipeline figure deferred until demo_pipeline.png is generated from
% examples/segmentation_workflow.ipynb. Uncomment when image is ready.
%\added{%
%\begin{figure*}[!ht]
%\centering
%\includegraphics[width=\textwidth]{../assets/demo_pipeline.png}
%\caption{%
%  End-to-end pipeline: confocal nuclei segmentation to shape clustering.
%  (\textbf{a}) Raw 3D confocal fluorescence image (nuclei channel).
%  (\textbf{b}) Segmented nuclei as an integer label image; each colour represents a distinct nucleus.
%  (\textbf{c}) PCA of the full Minkowski tensor feature vector computed for all nuclei in a single \texttt{minkowski\_tensors\_from\_label\_image} call.
%  (\textbf{d}) 3D meshes of segmented nuclei coloured by shape cluster.
%  The complete pipeline is provided in \texttt{examples/segmentation\_workflow.ipynb}.
%  \textit{Alt text: four-panel figure. Top-left: grey-scale 3D confocal image. Top-right: colour-coded integer segmentation. Bottom-left: 2D PCA scatter coloured by cluster. Bottom-right: 3D mesh render coloured by cluster.}
%}
%\label{fig:workflow}
%\end{figure*}
%}
...
\caption{%
  ...
  Notation: $K$ denotes the body, $\partial K$ its triangulated surface, $f$ a face with area $A_f$ and normal $\hat{n}_f$, $e$ an edge with length $\ell_e$ and dihedral angle $\alpha_e$, and $v$ a vertex with angle deficit $\delta_v$.
  See \texttt{docs/minkowski\_tensors.md} for discrete mesh formulae.
}
\label{tab:tensors}
\small
\begin{tabular}{llll}
...
\texttt{w300} & $W_3^{0,0}$ & $\frac{2\pi}{3}\chi$ (Gauss--Bonnet) & Euler characteristic \\
\hline
\multicolumn{4}{l}{\textit{Vectors} $W_\nu^{1,0}$} \\
\texttt{w010} & $W_0^{1,0}$ & $\int_K \mathbf{x}\,dV$ & Volume moment \\
\texttt{w110} & $W_1^{1,0}$ & $\frac{1}{3}\int_{\partial K} \mathbf{x}\,dA$ & Surface moment \\
\texttt{w210} & $W_2^{1,0}$ & $\frac{1}{3}\int_{\partial K} H\,\mathbf{x}\,dA$ & Curvature-weighted centroid \\
\texttt{w310} & $W_3^{1,0}$ & $\frac{1}{3}\int_{\partial K} K_G\,\mathbf{x}\,dA$ & Gauss-curvature-weighted centroid \\
\hline
...
\hline
\multicolumn{4}{l}{\textit{Higher-rank} (\texttt{compute=`all'})} \\
...
\texttt{msm\_ql} & $q_\ell$ & $\sqrt{\frac{4\pi}{2\ell+1}\sum_m|Q_{\ell m}|^2}$ & Orientational order strength \\
\texttt{msm\_wl} & $w_\ell$ & $\propto\tilde{w}_\ell^{1/3}$ (Wigner $3j$) & Phase of orientational order \\
\hline
\end{tabular}
\end{table*}
}
