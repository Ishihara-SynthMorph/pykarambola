...
\replaced{Validated on synthetic meshes spanning three topological classes (a genus-1 torus, a genus-2 holey slab, and a hollow shell) and benchmarked}{Benchmarked} on 1{,}584 adrenal gland meshes, pykarambola reproduces all 121 C++ karambola output features to near-floating-point agreement and, in the pure-Python build, is $2.8\times$ faster at $28^3$ and $1.5\times$ faster at $64^3$ voxel resolution, with speedups primarily attributable to karambola's sequential per-object file I/O.
...
\caption{%
  ...
  \added{\textit{Alt text: flow diagram with three input paths on the left (mesh files, Triangulation objects, and 3D label images) converging on a central compute core, which outputs three families of shape descriptors on the right: scalars, vectors, and rank-2 tensors, with optional higher-rank and irreducible quantities.}}
}
\label{fig:pipeline}
\end{figure*}
...
\replaced{We extended this accuracy assessment to the full 121-feature output on a large biological dataset, comparing \texttt{pykarambola} against C++ \texttt{karambola} on the AdrenalMNIST3D d}
...
\added{Supplementary Figure~\ref{fig:workflow} demonstrates a complete end-to-end segmentation-to-morphometry pipeline applied to 3D confocal nuclei (\texttt{skimage.data.cells3d}~\cite{vanderwalt2014scikit}): a raw 60-slice fluorescence stack is segmented into 12 individually resolved nuclei via scikit-image Gaussian smoothing, Otsu thresholding, and watershed, Minkowski tensors are computed for all segmented nuclei in a single \texttt{minkowski\_tensors\_from\_label\_image} call, and from the full tensor output, the rotationally invariant subset (Minkowski scalars, $\beta$, eigenvalues, trace, and MSM $q_l/w_l$; 56 features per nucleus) is visualised by PCA projection and partitioned by hierarchical agglomerative clustering into three morphological groups.
One cluster isolates the mitotic cell (lower $\beta$, larger volume); the remaining two separate the interphase nuclei by degree of elongation.
The full pipeline is provided in \texttt{examples/segmentation\_workflow.ipynb} in the project repository.}

To illustrate how labelling choice determines what Minkowski tensors measure for a multi-object biological structure, we applied the label-image API to a publicly available 3D fluorescence segmentation of a human induced pluripotent stem cell (hiPSC) in early anaphase/telophase from the Allen Cell Collection~\cite{viana2023integrated}.
...
\added{Supplementary Figure~\ref{fig:workflow} extends this to a full population-level morphometry use case: watershed segmentation of a 60-slice confocal stack, per-nucleus Minkowski tensor computation in a single API call, and unsupervised shape clustering are chained in under 50 lines of Python, with the resulting PCA projection recovering biologically interpretable morphotypes including a mitotic cell.}
...
\replaced{For smooth objects, tensor values converge asymptotically as resolution increases.
However, $W_\nu^{r,s}$ with $\nu \neq 0$ (surface-integral-based tensors) are subject to an orientation bias from the anisotropy of the digitized data that can be significant at typical fluorescence microscopy resolutions \cite{Svane2017}.}{For smooth objects, tensor values converge asymptotically as voxel resolution increases, but rank-2 surface tensors are subject to an orientation bias from the anisotropy of the digital grid that can be significant at typical fluorescence microscopy resolutions \cite{Svane2017};}
\added{To quantify this effect, we rotated a synthetic triaxial ellipsoid (semi-axes $A=10$, $B=6$, $C=4$, in arbitrary distance units) through 12 orientations (0\textdegree--165\textdegree{} in 15\textdegree{} steps) at 7 resolutions spanning 20 to 100 voxels per semi-axis~$A$, and measured the systematic bias (mean deviation of $\beta$ over orientations relative to the fine-resolution reference) and orientation spread (standard deviation of $\beta$ over orientations) for all six rank-2 tensors and all four scalar Minkowski functionals (Supplementary Figure~\ref{fig:orientation_bias}).
For z-axis rotation at 20 voxels per semi-axis~$A$, the orientation spread in $\beta$ ranges from 0.3\% for $W_0^{2,0}$ to 1.5\% for $W_3^{2,0}$, measured relative to the zero-angle fine-resolution ($A/100$) reference.
This orientation-dependent bias is an inherent property of marching-cubes surface extraction.}
\replaced{An}{an} alternative Voronoi-based approach that is asymptotically unbiased for binary voxel inputs without requiring surface reconstruction has recently been proposed \cite{Hug2025}.
...
\centering
\caption{%
  \added{Mathematical definitions of Minkowski tensors computed by pykarambola.
  Notation: $K$ denotes the body with boundary $\partial K$; $\mathbf{x}$ is a position vector, $n_i$ the $i$-th component of the unit outward surface normal, $H=(\kappa_1{+}\kappa_2)/2$ the mean curvature, $K_G$ the Gaussian curvature, and $\chi$ the Euler characteristic.
  See \texttt{docs/minkowski\_tensors.md} for discrete formulae and for full definitions of the Minkowski Structure Metrics ($q_\ell$, $w_\ell$) returned by \texttt{msm\_ql} and \texttt{msm\_wl} with \texttt{compute=`all'}.}
}
\label{tab:tensors}
\small
...
\hline
\multicolumn{4}{l}{\textit{Higher-rank tensors and Minkowski Structure Metrics} (\texttt{compute=`all'})} \\
...
\texttt{msm\_ql}, \texttt{msm\_wl} & $q_\ell,\,w_\ell$ & \multicolumn{2}{l}{Minkowski Structure Metrics — see \texttt{docs/minkowski\_tensors.md}} \\
\hline
\end{tabular}
\end{table*}
}
...
\caption{%
  \added{Numerical accuracy of \texttt{pykarambola} relative to C++ \texttt{karambola} on three synthetic meshes of known topology.
  ...
  Rows correspond to shapes: torus ($\chi = 0$, genus~1), holey slab (rectangular slab with two rectangular through-holes, $\chi = -2$, genus~2), and hollow shell ($\chi = +4$, two components).
...
\added{\textit{Alt text: same layout as Supplementary Figure~\ref{fig:accuracy} at $64^3$ voxel resolution; all points lie on the red diagonal indicating perfect agreement between the two implementations.}}}
...
\caption{%
  \added{End-to-end segmentation-to-morphometry pipeline applied to 3D confocal nuclei (\texttt{skimage.data.cells3d}; data provided by the Allen Institute for Cell Science~\cite{vanderwalt2014scikit}).
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{notebooks/results/orientation_bias_bias_sigma_pct_rot_z.pdf}
\caption{%
  \added{Orientation bias introduced by marching-cubes surface extraction on a regular voxel grid, expressed as percentage deviation from the fine-resolution reference.
  A synthetic triaxial ellipsoid (semi-axes $A=10$, $B=6$, $C=4$, in arbitrary distance units) was rotated through 12 orientations (0\textdegree--165\textdegree{} in 15\textdegree{} steps) at 7 resolutions spanning 20 to 100 voxels per semi-axis~$A$.
  Solid lines show the absolute systematic bias $(|\bar{\beta}-\beta_{\mathrm{ref}}|/\beta_{\mathrm{ref}})\times 100\%$ (mean over 12 orientations); shaded bands show the absolute bias $\pm\sigma/\beta_{\mathrm{ref}}\times 100\%$ (orientation spread).
  \textbf{(a)} Bias and spread for each of the six rank-2 Minkowski tensors ($W_\nu^{r,s}$).
  \textbf{(b)} The same quantities for three scalar Minkowski functionals ($W_0^{0,0}$, $W_1^{0,0}$, $W_2^{0,0}$).
  $W_3^{0,0}$ (Euler characteristic) is a topological invariant preserved exactly by marching cubes at all resolutions and orientations (bias and spread $<10^{-12}$\%) and is therefore omitted.
  \textit{Alt text: two-panel figure. Panel (a): a 2-by-3 grid of line plots showing percentage systematic bias (solid line) and percentage orientation spread (shaded band) for six rank-2 Minkowski tensors as a function of voxels per semi-axis (log scale); bias and spread decrease with increasing resolution. Panel (b): a row of three plots showing percentage bias and spread for three scalar Minkowski functionals.}}
}
