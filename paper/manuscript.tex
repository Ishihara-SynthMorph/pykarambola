...
A C++ implementation, karambola~\cite{Schaller2020karambola}, computes Minkowski tensors for triangulated surfaces but is inaccessible within Python-based bioimage workflows: it requires a C++ build toolchain, is not distributed through a package manager, and provides no Python-native interface.
Here we present pykarambola, a Python package installable via \texttt{pip install pykarambola} that accepts NumPy arrays and standard mesh formats and returns Minkowski tensors — including derived anisotropy and orientation quantities — as plain Python dicts.
A high-level label-image API (\texttt{minkowski\_tensors\_from\_label\_image}) converts 3D integer segmentation arrays into per-object Minkowski tensors in a single call, making pykarambola directly compatible with standard segmentation tools such as cellpose and StarDist.
...
Where pure-Python performance is insufficient for large meshes, an optional Cython~\cite{behnel2011cython} extension accelerates the graph-traversal steps of mesh initialisation; the package falls back to pure-Python implementations when the extension is not installed (see Acceleration strategy, below).
...
\caption{%
  ...
  centers each mesh;
  mesh files (\texttt{.poly}, \texttt{.off}, \texttt{.obj}, \texttt{.glb}, \texttt{.stl}) or direct
  ...
  scalars ($W_0^{0,0}$, $W_1^{0,0}$, $W_2^{0,0}$, $W_3^{0,0}$),
  vectors ($W_\nu^{1,0}$),
  and rank-2 tensors ($W_\nu^{2,0}$ and $W_\nu^{0,2}$) with their eigensystems
  and anisotropy index~$\beta$.
  Derived scalar quantities may involve normalization by a scalar Minkowski functional; see~\cite{SchroderTurk2013} for exact definitions.
}
\label{fig:pipeline}
\end{figure*}
...
rank-3 tensor $W_1^{0,3}$ (\texttt{w103}), 6 eigenvalues of $W_1^{0,4}$ 
...
eigenvalue solver with \texttt{numpy.linalg.eigh}. The maximum mean absolute 
