...
Both implementations converge to the analytical scalar values with increasing resolution (Supplementary Figure~\ref{fig:topology}); closed forms are not generally available for the higher-rank tensors on these geometries, so for those we confirm that pykarambola reproduces karambola to near floating-point precision.}
...
We note that Minkowski tensors are subject to an orientation bias from the anisotropy of the digitized data that can be significant at lower resolutions (Supplementary Figure~\ref{fig:orientation_bias}) \cite{Svane2017}.}
...
\caption{%
  \added{Resolution convergence of \texttt{pykarambola} and C++ \texttt{karambola} toward analytical Minkowski scalar values on three synthetic meshes of known topology.
  Each panel plots one scalar Minkowski functional against mesh resolution~$n$ for karambola (green, solid, filled circles), pykarambola (blue, dashed, open squares), and, and the analytical value (red dash-dot line).
  ...
  Columns correspond to the four scalar Minkowski functionals: volume $W_0^{0,0}$, surface area $W_1^{0,0}$, integrated mean curvature $W_2^{0,0}$, and the topological term $W_3^{0,0}$ ($\propto \chi$).
  ...
  \textit{Alt text: a 3-by-4 grid of line plots. Rows are torus, holey slab, and hollow shell; columns are the volume, surface area, integrated mean curvature, and topological scalar Minkowski functionals. In each panel the horizontal axis is mesh resolution and the vertical axis is the functional value; a green solid curve (karambola) and a blue dashed curve (pykarambola) overlie one another and approach a red dash-dot horizontal line marking the analytical value as resolution increases. The topological-term panels are flat, lying on the analytical value at all resolutions.}}
}
\label{fig:topology}
\end{figure*}
}
