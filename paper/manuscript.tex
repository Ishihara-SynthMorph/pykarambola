...
(Figure~\ref{fig:accuracy}q--t). For \texttt{msm\_ql} and even-$l$ 
\texttt{msm\_wl}, both implementations agree to floating-point precision 
(Figure~\ref{fig:accuracy}q--s). For odd-$l$ \texttt{msm\_wl} ($l = 1, 3, 5, 7$), 
the Wigner 3j parity selection rule requires $w_l = 0$ exactly 
(Figure~\ref{fig:accuracy}t). \texttt{pykarambola}'s Racah-formula implementation 
applies this rule analytically and returns exactly zero, whereas C++ 
\texttt{karambola} uses \texttt{gsl\_sf\_coupling\_3j}, which evaluates 
...
vectors ($W_\nu^{1,0}$, panels e--h), rank-2 tensors ($W_\nu^{2,0}$, 
$W_\nu^{0,2}$, panels i--n), and higher-rank quantities including $W_1^{0,3}$, 
$W_1^{0,4}$ eigenvalues, and spherical Minkowski summary quantities ($q_l$, 
$w_l$, panels o--t).
The red diagonal indicates perfect agreement; all points fall on this line.
Equivalent agreement is observed at $64^3$ resolution (Figure~\ref{fig:accuracy_supp}).}
\label{fig:accuracy}
\end{figure*}
...
All panels and groupings are identical to Figure~\ref{fig:accuracy}; all points fall on the identity line.}
