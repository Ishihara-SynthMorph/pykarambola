...
For smooth objects, tensor values converge asymptotically as resolution increases.
However, $W_\nu^{r,s}$ with $\nu \neq 0$ (surface-integral-based tensors) are subject to an orientation bias from the anisotropy of the digitized data that can be significant at typical fluorescence microscopy resolutions \cite{Svane2017}.
\added{To quantify this effect, we rotated a synthetic triaxial ellipsoid (semi-axes $A=10$, $B=6$, $C=4$~$\mu$m) through 12 orientations (0\textdegree--165\textdegree{} in 15\textdegree{} steps) at 12 voxel sizes ($A/3$ to $A/100$) and measured the systematic bias and orientation spread in $\beta$ for all six rank-2 tensors and all four scalar Minkowski functionals (Supplementary Figure~\ref{fig:orientation_bias}).
Scalar functionals converge monotonically without orientation dependence.
At 20 voxels per major semi-axis the spread in $\beta$ (standard deviation over 12 orientations, normalised by the fine-resolution reference) ranges from 0.3\% for $W_0^{2,0}$ to 1.5\% for $W_3^{2,0}$; at 3 voxels per semi-axis these grow to 56\% and 52\%, respectively.
...
An alternative Voronoi-based approach that is asymptotically unbiased for binary voxel inputs without requiring surface reconstruction has recently been proposed \cite{Hug2025}.
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{notebooks/results/orientation_bias_bias_sigma_rot_z.pdf}
\caption{Orientation bias introduced by marching-cubes surface extraction on a regular voxel grid.
A synthetic triaxial ellipsoid (semi-axes $A=10$, $B=6$, $C=4$~$\mu$m) was rotated through 12 orientations (0\textdegree--165\textdegree{} in 15\textdegree{} steps) at 12 voxel sizes ($A/3$ to $A/100$).
Solid lines show the systematic bias ($\bar{\beta}-\beta_{\mathrm{ref}}$, mean over 12 orientations minus fine-resolution reference); shaded bands show $\pm\sigma$ (standard deviation over 12 orientations); dashed lines mark zero.
\textbf{(a)} Bias and spread for each of the six rank-2 Minkowski tensors ($W_\nu^{r,s}$).
\textbf{(b)} The same quantities for the four scalar Minkowski functionals; scalars converge monotonically with negligible orientation spread.
\textit{Alt text: two-panel figure. Panel (a): a 2-by-3 grid of line plots showing systematic bias (solid line) and orientation spread (shaded band) for six rank-2 Minkowski tensors as a function of voxels per semi-axis (log scale); bias and spread decrease with increasing resolution. Panel (b): a row of four plots showing bias and spread for the four scalar Minkowski functionals; the scalars converge smoothly with negligible spread across orientations.}}
