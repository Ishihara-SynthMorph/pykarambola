...
The speedup \emph{ratio} is largest at smaller mesh sizes, where the graph-traversal routines account for a greater fraction of total compute time; at larger mesh sizes the NumPy-vectorised array operations dominate and the Cython gain becomes proportionally smaller (Amdahl's Law).
The absolute time saved by Cython, however, grows with mesh size --- from $\sim$0.03~s at subdivision~5 to $\sim$27~s at subdivision~10.
