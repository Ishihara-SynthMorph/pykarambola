...
The \texttt{minkowski\_tensors\_from\_label\_image} function accepts a 3D integer NumPy array
in which each nonzero value identifies a distinct object,
and returns a dict keyed by label with the same Minkowski tensor outputs as the mesh API
(Figure~\ref{fig:pipeline}).
Internally, the function calls marching cubes on each label's binary mask to extract
a triangulated isosurface, then pipes the resulting mesh through the standard
\texttt{minkowski\_tensors} computation.
The labelling strategy determines what the tensors measure:
assigning all foreground voxels a single label (Figure~\ref{fig:labelimage}a) yields
one set of tensors describing the aggregate structure,
whereas splitting connected components with \texttt{scipy.ndimage.label} and assigning
each a unique integer label (Figure~\ref{fig:labelimage}b) produces per-object tensors
that resolve individual morphologies.

\begin{figure}
\centering
\includegraphics[width=\linewidth]{../assets/pipeline_figure18.pdf}
\caption{%
