...
The spherical Minkowski summary quantities (\texttt{msm\_ql\_$l$} and \texttt{msm\_wl\_$l$} for $l = 0, \ldots, 8$), available in \texttt{compute='all'} mode, show a distinct accuracy pattern.
For all \texttt{msm\_ql} values and for even-$l$ \texttt{msm\_wl}, pykarambola and C++ karambola agree to floating-point precision.
For odd-$l$ \texttt{msm\_wl} ($l = 1, 3, 5, 7$), however, the Wigner 3j parity selection rule requires $w_l = 0$ identically whenever $J = j_1 + j_2 + j_3 = 3l$ is odd.
pykarambola returns exactly zero for these entries because the pure-Python Racah-formula implementation of the Wigner 3j coefficient applies this selection rule analytically.
The C++ implementation uses \texttt{gsl\_sf\_coupling\_3j}, which evaluates the coefficient numerically without an explicit parity check, producing spurious floating-point residuals of order $10^{-7}$--$10^{-8}$ on the AdrenalMNIST3D meshes (standard deviations across 1{,}584 shapes: $w_3$: $1.0 \times 10^{-7}$; $w_5$: $6.7 \times 10^{-8}$; $w_7$: $8.0 \times 10^{-8}$).
The residuals arise because complex biological shapes have non-negligible odd-$l$ spherical harmonic coefficients $d_{lm}$ (of order $10^{-2}$--$10^{-1}$, consistent with $q_3 \approx 0.08$), so numerical errors in the 3j coefficients accumulate across the triple sum over $(m_a, m_b, m_c)$ rather than cancelling as they do for near-spherical geometries.
This improvement was an indirect consequence of removing the \texttt{sympy} dependency: both the original \texttt{sympy} implementation and the Racah-formula replacement apply the parity rule analytically, whereas the C++ GSL backend does not.
