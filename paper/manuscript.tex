...
Benchmarked against C++ karambola on 1{,}584 adrenal gland meshes from the AdrenalMNIST3D dataset, pykarambola is $2.8\times$ faster at $28^3$ resolution and $1.5\times$ faster at $64^3$ resolution, with the speedup primarily attributable to C++ karambola's sequential per-object file I/O; the Cython-accelerated build achieves $1.0$--$1.2\times$ speedup relative to C++ karambola on synthetic icospheres.
Numerical output agrees with C++ karambola to floating-point precision across all 121 scalar, vector, and tensor features evaluated.
