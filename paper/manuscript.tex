...
\replaced{Numerical correctness of the port was further validated by benchmarking against the C++ karambola binary on synthetic meshes spanning three topological classes (torus, holey slab, hollow shell) with known analytical Minkowski values, and on a large biological dataset, confirming near-floating-point agreement for all Minkowski scalars, vectors, and tensors.}{Numerical correctness of the port was further validated by extensive benchmarking against the C++ karambola binary across a diverse set of test meshes, confirming near-floating-point agreement for all Minkowski scalars, vectors, and tensors.}
...
We benchmarked pykarambola\added{'s runtime} against the reference C++ karambola binary on two mesh sets.
...
\added{We benchmark pykarambola against the original karambola implementation on both synthetic shapes, where analytical values provide independent ground truth, and biological datasets representative of real use. 
To test of correctness on topologically non-trivial synthetic surfaces, we evaluated \texttt{pykarambola} and C++ \texttt{karambola} on three synthetic meshes with known analytical Minkowski scalar values: a parametric torus ($\chi = 0$, genus~1), a rectangular slab with two rectangular through-holes ($\chi = -2$, genus~2), and two nested icospheres forming a hollow shell ($\chi = +4$; genus = 0).
Each shape was tested at five mesh resolutions.
Across all shapes, resolutions, and feature types (scalars, vectors, rank-2 tensors), both implementations fall on the identity line (Supplementary Figure~\ref{fig:topology}), confirming that the reimplementation handles non-trivial topology identically to the reference..
At the finest resolution, the scalar functionals converge to within 0.08\% of their analytical values for all shapes and $W_3^{0,0}$ reproduces the exact topological value $(2\pi/3)\chi$ to within $10^{-11}$.
...
\replaced{We extended this accuracy assessment to the full 121-feature output on a large biological dataset, comparing \texttt{pykarambola} against C++ \texttt{karambola} on the AdrenalMNIST3D }{To assess numerical accuracy, we compared all 121 output features of \texttt{pykarambola} against C++ \texttt{karambola} on the full AdrenalMNIST3D }
...
\added{Supplementary Figure~\ref{fig:workflow} embeds the label-image API in a complete, biologist-facing workflow. A raw 3D confocal stack (\texttt{skimage.data.cells3d}~\cite{vanderwalt2014scikit}) is first segmented into individually labelled nuclei using standard scikit-image tools; pykarambola then takes this integer label image as input and returns per-object Minkowski tensors in a single \texttt{minkowski\_tensors\_from\_label\_image} call. 
Clustering the nuclei on their rotation-invariant shape features recovers biologically meaningful morphotypes, cleanly separating the mitotic cell (lower~$\beta$, larger volume) from the interphase population.}

\replaced{To illustrate how labelling choice determines what Minkowski tensors measure for a multi-object biological structure, we applied the label-image API}{To demonstrate the API, we applied it} to a publicly available 3D fluorescence segmentation of a human induced pluripotent stem cell (hiPSC) in early anaphase/telophase from the Allen Cell Collection~\cite{viana2023integrated}.
...
\newtxt{Users encountering open or non-manifold meshes in their own datasets can repair them using established mesh-processing tools such as MeshLab~\cite{Cignoni2008meshlab} prior to passing the mesh to pykarambola.}
...
\caption{%
  ...
  Columns correspond to feature groups: scalars ($W_\nu^{0,0}$), vectors ($W_\nu^{1,0}$), rank-2 tensors ($W_\nu^{2,0}$ and $W_\nu^{0,2}$), and Minkowski structure metrics (MSM) $q_l$ and $w_l$.
  The red diagonal indicates perfect agreement; all scalar, tensor and MSM points fall on this line.
  ...
  \textit{Alt text: a 3-by-4 grid of scatter plots. Rows correspond to torus, holey slab, and hollow shell; columns correspond to scalar, vector, rank-2 tensors, and Minkowski structure metrics. In each panel the $x$-axis shows karambola values and the $y$-axis shows pykarambola values; a red diagonal line marks perfect agreement. Scalar, tensor and structure metric panels show points tightly clustered on the diagonal. Vector panels show points scattered near the origin, reflecting values close to zero for centrosymmetric shapes.}}
}
\label{fig:topology}
\end{figure*}
}
...
\caption{%
  \added{Orientation dependence of Minkowski measures under grid rotation. 
...
The dotted grey line is the fine-resolution reference, evaluated at the finest grid (100 voxels per~$A$) at $0^\circ$ orientation.
...
(\textbf{b})~Value of the three scalar Minkowski functionals $W_0^{0,0}$ (volume), $W_1^{0,0}$ (surface area), and $W_2^{0,0}$ (integrated mean curvature); the topological functional $W_3^{0,0}$ is omitted as it is aninteger invariant unaffected by orientation and resolution.
...
All curves oscillate periodically with angle; the oscillation amplitude shrinks and the curves approach a dotted grey reference line as resolution increases.}}
}
